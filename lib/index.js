...
// —— 戳一戳(petpet) / 彩蛋动画:配置 + 素材库 ——
// 配置独立成 .dshw-pet.json,不塞进 size.json:size.json 的读/写是「缺字段沿用现值」的
// 白名单式实现(issue #97),往里加字段要同时改读写两处,且会被旧客户端整包洗掉。
const PET_FILE_CANDIDATES = [
  path.join(DSH_HOME, '.dshw-pet.json'),
  path.join(DSH_HOME, 'profiles', 'web', '.dshw-pet.json'),
]
const PET_DIR_CANDIDATES = [
  path.join(DSH_HOME, 'whale-pet'),
  path.join(DSH_HOME, 'profiles', 'web', 'whale-pet'),
]
const PET_INDEX_NAME = 'assets.json'
const PET_ASSET_MAX_BYTES = 8 * 1024 * 1024
// 请求体上限:图片按 base64 传(data URL),体积约 4/3,再留 JSON 包装与余量 ——
// 直接拿 8MB 当 body 上限会把「接近 8MB 的合法图片」误杀。
const PET_BODY_MAX_BYTES = Math.ceil(PET_ASSET_MAX_BYTES * 4 / 3) + 2 * 1024 * 1024
const PET_FORMAT_MIME = {
  png: 'image/png',
  gif: 'image/gif',
  webp: 'image/webp',
  apng: 'image/apng',
...
// 内置素材:kind 只有两种 —— pet(戳一戳图层素材) / fx(彩蛋素材:铁砧 / 经验球 / 装备)
const BUILTIN_PET_ASSETS = [
  { id: 'builtin_petpet_template', name: '经典拍头(手)', kind: 'pet', file: 'petpet-template.gif', format: 'gif' },
  { id: 'builtin_basin', name: '不锈钢盆', kind: 'fx', file: 'fx-basin.png', format: 'png' },
  { id: 'builtin_anvil', name: 'Minecraft 铁砧(原版)', kind: 'fx', file: 'fx-anvil.png', format: 'png' },
  { id: 'builtin_exp_orb', name: '经验球(原版)', kind: 'fx', file: 'fx-exp-orb.png', format: 'png' },
  { id: 'builtin_smoke', name: 'MC 烟雾粒子(原版)', kind: 'fx', file: 'fx-smoke.png', format: 'png' },
  { id: 'builtin_item_sword', name: '钻石剑(原版)', kind: 'fx', file: 'fx-item-sword.png', format: 'png' },
  { id: 'builtin_item_pickaxe', name: '钻石镐(原版)', kind: 'fx', file: 'fx-item-pickaxe.png', format: 'png' },
  { id: 'builtin_item_apple', name: '金苹果(原版)', kind: 'fx', file: 'fx-item-apple.png', format: 'png' },
  { id: 'builtin_item_ingot', name: '金锭(原版)', kind: 'fx', file: 'fx-item-ingot.png', format: 'png' },
  { id: 'builtin_item_emerald', name: '绿宝石(原版)', kind: 'fx', file: 'fx-item-emerald.png', format: 'png' },
]
function builtinPetAssetBytes(def) {
  if (!def || !def.file) return null
  try {
    const bytes = fs.readFileSync(path.join(PACKAGE_ROOT, 'assets', def.file))
    if (bytes && bytes.length > 0) return bytes
  } catch (err) {}
  return null
}
// 自定义脚本轨道白名单(前端 DSL 只认这 5 条轨道)
const PET_SCRIPT_TRACKS = ['x', 'y', 'scale', 'rotate', 'opacity']
const PET_CONFIG_DEFAULTS = {
  pet: {
    enabled: true,
    assetId: 'builtin_petpet_template',
    animId: 'builtin:classic',
    mode: 'overlay',
    randomScript: false,
    stageOnPress: true,
    anchor: 'avatar',
  ...
  fx: {
    enabled: true,
    prob: 1,
    animId: 'builtin:anvil',
    anvilId: 'builtin_basin',
    orbId: 'builtin_exp_orb',
    smokeId: 'builtin_smoke',
    itemIds: ['builtin_item_sword', 'builtin_item_pickaxe', 'builtin_item_apple', 'builtin_item_ingot', 'builtin_item_emerald'],
    scale: 1,
    x: 0,
    y: 0,
    orbCount: 0,
...
function petClampNum(v, lo, hi) { return v < lo ? lo : (v > hi ? hi : v) }
// 自定义脚本只做**结构校验 + 长度/范围夹紧**,不解释内容(解释在前端的受限 DSL 解析器里,
// 不用 eval)。非法条目直接丢弃,不阻断整份配置的保存。
function sanitizePetScript(rawScripts, fallback) {
  const src = Array.isArray(rawScripts) ? rawScripts : fallback
  const out = []
  if (!Array.isArray(src)) return out
  for (const s of src.slice(0, 40)) {
    if (!s || typeof s !== 'object') continue
    const id = String(s.id || '').trim().slice(0, 64)
    const name = String(s.name || '').trim().slice(0, 40)
    if (!id || !name) continue
    const duration = Math.round(petClampNum(typeof s.duration === 'number' && isFinite(s.duration) ? s.duration : 700, 100, 20000))
    const loop = s.loop !== false
    const tracks = {}
    const srcTracks = s.tracks && typeof s.tracks === 'object' ? s.tracks : {}
    for (const key of PET_SCRIPT_TRACKS) {
      const t = srcTracks[key]
      if (!t || typeof t !== 'object') continue
      if (typeof t.expr === 'string' && t.expr.trim()) {
        const expr = t.expr.trim().slice(0, 400)
        if (/^[\x20-\x7e]+$/.test(expr)) tracks[key] = { expr }
        continue
      }
      if (Array.isArray(t.k)) {
        const k = []
        for (const pair of t.k.slice(0, 64)) {
          if (!Array.isArray(pair) || pair.length < 2) continue
          const tt = Number(pair[0])
          const vv = Number(pair[1])
          if (!isFinite(tt) || !isFinite(vv)) continue
          k.push([petClampNum(tt, 0, 1), petClampNum(vv, -1e5, 1e5)])
        }
        if (k.length) tracks[key] = { k }
      }
    }
    out.push({ id, name, duration, loop, tracks })
  }
  return out
}
// base = 现有值:缺字段一律沿用 base(PUT 只带改动字段也能安全落盘)
function normalizePetConfig(raw, base) {
  const cur = base && typeof base === 'object' ? base : {}
  const r = raw && typeof raw === 'object' ? raw : {}
  const pickB = (v, c, dflt) => (typeof v === 'boolean' ? v : (typeof c === 'boolean' ? c : dflt))
  const pickN = (v, c, dflt, lo, hi) => {
    const n = typeof v === 'number' && isFinite(v) ? v : (typeof c === 'number' && isFinite(c) ? c : dflt)
    return petClampNum(n, lo, hi)
  }
  const pickS = (v, c, dflt, allowed) => {
    let s = typeof v === 'string' && v ? v : (typeof c === 'string' && c ? c : dflt)
    if (allowed && allowed.indexOf(s) === -1) s = allowed.indexOf(dflt) === -1 ? allowed[0] : dflt
    return s
  }
  const petRaw = r.pet && typeof r.pet === 'object' ? r.pet : {}
  const fxRaw = r.fx && typeof r.fx === 'object' ? r.fx : {}
  const petCur = cur.pet && typeof cur.pet === 'object' ? cur.pet : {}
  const fxCur = cur.fx && typeof cur.fx === 'object' ? cur.fx : {}
  const d = PET_CONFIG_DEFAULTS
  const itemSrc = Array.isArray(fxRaw.itemIds) ? fxRaw.itemIds : (Array.isArray(fxCur.itemIds) ? fxCur.itemIds : d.fx.itemIds)
  const itemIds = itemSrc.slice(0, 16).map((x) => String(x || '').slice(0, 64)).filter(Boolean)
  return {
    v: 1,
    pet: {
      enabled: pickB(petRaw.enabled, petCur.enabled, d.pet.enabled),
      assetId: String(pickS(petRaw.assetId, petCur.assetId, d.pet.assetId)).slice(0, 64),
      animId: String(pickS(petRaw.animId, petCur.animId, d.pet.animId)).slice(0, 64),
      mode: pickS(petRaw.mode, petCur.mode, d.pet.mode, ['composite', 'overlay']),
      randomScript: pickB(petRaw.randomScript, petCur.randomScript, d.pet.randomScript),
      stageOnPress: pickB(petRaw.stageOnPress, petCur.stageOnPress, d.pet.stageOnPress),
      anchor: pickS(petRaw.anchor, petCur.anchor, d.pet.anchor, ['avatar', 'widget']),
      x: pickN(petRaw.x, petCur.x, d.pet.x, -200, 200),
      y: pickN(petRaw.y, petCur.y, d.pet.y, -200, 200),
      scale: pickN(petRaw.scale, petCur.scale, d.pet.scale, 0.1, 5),
      rotate: pickN(petRaw.rotate, petCur.rotate, d.pet.rotate, -180, 180),
      opacity: pickN(petRaw.opacity, petCur.opacity, d.pet.opacity, 0.05, 1),
      avatarScale: pickN(petRaw.avatarScale, petCur.avatarScale, d.pet.avatarScale, 0.1, 5),
      avatarX: pickN(petRaw.avatarX, petCur.avatarX, d.pet.avatarX, -200, 200),
      avatarY: pickN(petRaw.avatarY, petCur.avatarY, d.pet.avatarY, -200, 200),
      loops: Math.round(pickN(petRaw.loops, petCur.loops, d.pet.loops, 1, 20)),
      speed: pickN(petRaw.speed, petCur.speed, d.pet.speed, 0.2, 4),
      sound: pickB(petRaw.sound, petCur.sound, d.pet.sound),
    },
    fx: {
      enabled: pickB(fxRaw.enabled, fxCur.enabled, d.fx.enabled),
      prob: pickN(fxRaw.prob, fxCur.prob, d.fx.prob, 0, 100),
      animId: String(pickS(fxRaw.animId, fxCur.animId, d.fx.animId)).slice(0, 64),
      anvilId: String(pickS(fxRaw.anvilId, fxCur.anvilId, d.fx.anvilId)).slice(0, 64),
      orbId: String(pickS(fxRaw.orbId, fxCur.orbId, d.fx.orbId)).slice(0, 64),
      smokeId: String(pickS(fxRaw.smokeId, fxCur.smokeId, d.fx.smokeId)).slice(0, 64),
      itemIds: itemIds.length ? itemIds : d.fx.itemIds.slice(),
      scale: pickN(fxRaw.scale, fxCur.scale, d.fx.scale, 0.2, 4),
      x: pickN(fxRaw.x, fxCur.x, d.fx.x, -200, 200),
      y: pickN(fxRaw.y, fxCur.y, d.fx.y, -200, 200),
      orbCount: Math.round(pickN(fxRaw.orbCount, fxCur.orbCount, d.fx.orbCount, 0, 60)),
      smokeCount: Math.round(pickN(fxRaw.smokeCount, fxCur.smokeCount, d.fx.smokeCount, 0, 60)),
      stageOnPress: pickB(fxRaw.stageOnPress, fxCur.stageOnPress, d.fx.stageOnPress),
      itemCount: Math.round(pickN(fxRaw.itemCount, fxCur.itemCount, d.fx.itemCount, 0, 30)),
      sound: pickB(fxRaw.sound, fxCur.sound, d.fx.sound),
      shake: pickB(fxRaw.shake, fxCur.shake, d.fx.shake),
    },
    scripts: sanitizePetScript(r.scripts, Array.isArray(cur.scripts) ? cur.scripts : d.scripts),
  }
}
const SOUND_SETS = {
...
export default {
  ...
  apply(ctx) {
    ...
    function usageRecordsPayload() {
      ...
      return {
        ok: true, version: '0.3.9', today: todayData, days7,
    ...
    function publicBalance(payload) {
      ...
      return {
        ...visible, version: '0.3.9', isPeak: isPeakTime(nowSec),
    ...
    // ===== 戳一戳(petpet) / 彩蛋动画:配置 + 素材库 =====
    // 与泡泡图库同构:用户素材落 $DSH_HOME/whale-pet/<id>.<ext> + assets.json 索引,
    // 内置素材直接从包内 assets/ 读 —— 前端只认 id,换素材不用改配置结构。
    function pickPetDir() {
      for (const p of PET_DIR_CANDIDATES) {
        try {
          fs.mkdirSync(p, { recursive: true })
          fs.accessSync(p, fs.constants.W_OK)
          return p
      ...
      return PET_DIR_CANDIDATES[0]
    }
    function readPetIndex() {
      try {
        const parsed = JSON.parse(fs.readFileSync(path.join(pickPetDir(), PET_INDEX_NAME), 'utf8'))
        if (parsed && Array.isArray(parsed.assets)) return parsed
      } catch (err) {}
      return { version: 1, assets: [] }
    }
    function writePetIndex(index) {
      try {
        fs.writeFileSync(path.join(pickPetDir(), PET_INDEX_NAME), JSON.stringify(index, null, 2), 'utf8')
        return true
      } catch (err) { return false }
    }
    function petAssetPayload() {
      const index = readPetIndex()
      const seen = {}
      const list = []
      for (const a of BUILTIN_PET_ASSETS) {
        seen[a.id] = 1
        list.push({ id: a.id, name: a.name, kind: a.kind, format: a.format, builtin: true })
      }
      for (const a of index.assets) {
        if (seen[a.id]) continue
        seen[a.id] = 1
        list.push({ id: a.id, name: a.name, kind: a.kind, format: a.format, builtin: false })
      }
      return { ok: true, assets: list }
    }
    function readPetConfigFile() {
      for (const p of PET_FILE_CANDIDATES) {
        try {
          const parsed = JSON.parse(fs.readFileSync(p, 'utf8'))
    ...
    function readPetConfig() {
      return normalizePetConfig(readPetConfigFile(), PET_CONFIG_DEFAULTS)
    }
    function writePetConfig(cfg) {
      const body = JSON.stringify(Object.assign({}, cfg, { updatedAt: new Date().toISOString() }), null, 2)
      for (const p of PET_FILE_CANDIDATES) {
        try {
          fs.writeFileSync(p, body, 'utf8')
          return true
    ...
    function petAssetBytesById(id) {
      const index = readPetIndex()
      const user = index.assets.find((x) => x.id === id)
      if (user && PET_FORMAT_MIME[user.format]) {
        try {
          return { bytes: fs.readFileSync(path.join(pickPetDir(), user.id + '.' + user.format)), format: user.format }
        } catch (err) {}
      }
      const def = BUILTIN_PET_ASSETS.find((x) => x.id === id)
      if (def) {
        const bytes = builtinPetAssetBytes(def)
        if (bytes) return { bytes, format: def.format }
      }
      return null
    }
    ...
    disposers.push(registerRoute({
      kind: 'exact',
      path: '/dsh-whale/pet.json',
      handler: async (req, res) => {
        try {
          if (req.method === 'PUT' || req.method === 'POST') {
            const body = await readBodyMax(req, 512 * 1024)
            const parsed = JSON.parse(body)
            const next = normalizePetConfig(parsed, readPetConfig())
            if (!writePetConfig(next)) {
              res.writeHead(500, JSON_HEADERS)
              res.end(JSON.stringify({ ok: false, error: 'pet config not writable' }))
              return
            }
            res.writeHead(200, JSON_HEADERS)
          ...
          res.writeHead(200, JSON_HEADERS)
          res.end(JSON.stringify({ ok: true, config: readPetConfig() }))
        } catch (err) {
          res.writeHead(400, JSON_HEADERS)
          res.end(JSON.stringify({ ok: false, error: String((err && err.message) || err).slice(0, 200) }))
        }
    ...
    disposers.push(registerRoute({
      kind: 'exact',
      path: '/dsh-whale/pet-assets.json',
      handler: async (req, res) => {
        if (req.method !== 'POST') {
          res.writeHead(200, JSON_HEADERS)
          res.end(JSON.stringify(petAssetPayload()))
          return
        }
        try {
          const body = await readBodyMax(req, PET_BODY_MAX_BYTES)
          const parsed = JSON.parse(body)
          const action = parsed && parsed.action
          const index = readPetIndex()
          if (action === 'upload') {
            const name = String(parsed.name || '').trim().slice(0, 40) || '素材'
            const kind = parsed.kind === 'fx' ? 'fx' : 'pet'
            const m = /^data:image\/(png|gif|webp|apng|jpeg|jpg);base64,([A-Za-z0-9+/=]+)$/.exec(String(parsed.data || ''))
            if (!m) {
              res.writeHead(400, JSON_HEADERS)
              res.end(JSON.stringify({ ok: false, error: 'invalid image data' }))
              return
            ...
            const buf = Buffer.from(m[2], 'base64')
            if (buf.length < 64 || buf.length > PET_ASSET_MAX_BYTES) {
              res.writeHead(400, JSON_HEADERS)
              res.end(JSON.stringify({ ok: false, error: 'image too large' }))
              return
            }
            const id = 'pet_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 8)
            fs.writeFileSync(path.join(pickPetDir(), id + '.' + format), buf)
            index.assets.push({ id, name, kind, format, createdAt: Date.now() })
            writePetIndex(index)
            res.writeHead(200, JSON_HEADERS)
            res.end(JSON.stringify(petAssetPayload()))
            return
          ...
          if (action === 'rename') {
            ...
            a.name = String(parsed.name || '').trim().slice(0, 40) || a.name
            writePetIndex(index)
            res.writeHead(200, JSON_HEADERS)
            res.end(JSON.stringify(petAssetPayload()))
            return
          ...
          if (action === 'delete') {
            ...
            index.assets = index.assets.filter((x) => x.id !== id)
            writePetIndex(index)
            try { fs.unlinkSync(path.join(pickPetDir(), a.id + '.' + a.format)) } catch (err) {}
            res.writeHead(200, JSON_HEADERS)
            res.end(JSON.stringify(petAssetPayload()))
            return
          }
    ...
    disposers.push(registerRoute({
      kind: 'exact',
      path: '/dsh-whale/pet-asset.png',
      handler: (req, res) => {
        try {
          const id = new URL(req.url || '/', 'http://localhost').searchParams.get('id') || ''
          const hit = petAssetBytesById(id)
          if (!hit) throw new Error('bad asset id')
          res.writeHead(200, {
            'Content-Type': PET_FORMAT_MIME[hit.format] || 'application/octet-stream',
            'Cache-Control': 'no-store',
