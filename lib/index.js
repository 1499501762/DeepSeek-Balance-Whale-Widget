...
export default {
  ...
  apply(ctx) {
    ...
    // v724 SSRF 防护：只允许 http/https，且禁止访问内网/本机地址（回环、私有网段、link-local、metadata 地址等）
    function assertSafeApiUrl(u) {
      ...
      if (host === 'localhost' || host === '0.0.0.0' || host === '::1' || host === '169.254.169.254') throw new Error('禁止访问该地址')
      ...
      if (/^f[cd][0-9a-f]{2}:/i.test(host) || /^fe80:/i.test(host)) throw new Error('禁止访问内网地址')
    }
